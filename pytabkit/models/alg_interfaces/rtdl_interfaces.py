...
class RTDL_MLPSubSplitInterface(SkorchSubSplitInterface):
    ...
    def get_required_resources(self, ds: DictDataset, n_cv: int, n_refit: int, n_splits: int,
        ...
        gpu_ram_params = {'': 0.4, 'ds_onehot_size_gb': 1.5,
                          'n_features': 1.5e-3 if self.config.get('module__num_emb_type', 'none') != 'none' else 1e-4}
...
class RTDL_MLP_ParamSampler:
    ...
    def sample_params(self, seed: int) -> Dict[str, Any]:
        ...
        if self.num_emb_type != 'none':
            params['module__num_emb_type'] = self.num_emb_type
            params['module__num_emb_dim'] = rng.choice(np.arange(1, 65))
            params['module__num_emb_hidden_dim'] = rng.choice(np.arange(1, 65))
            params['module__num_emb_sigma'] = np.exp(rng.uniform(np.log(1e-3), np.log(1e2)))
...
class RTDL_MLP_ParamSamplerNew:
    ...
    def sample_params(self, seed: int) -> Dict[str, Any]:
        ...
        params = {
            ...
            "module__d_embedding": rng.choice(np.arange(1, 65)),  # have smaller embedding sizes to avoid RAM issues
        ...
        if self.num_emb_type != 'none':
            params['module__num_emb_type'] = self.num_emb_type
            params['module__num_emb_dim'] = rng.choice(np.arange(1, 65))  # reduced from upper bound 128
            params['module__num_emb_hidden_dim'] = rng.choice(np.arange(1, 65))  # reduced from upper bound 128
            params['module__num_emb_sigma'] = np.exp(rng.uniform(np.log(1e-2), np.log(1e1)))
...
class RTDL_MLP_ParamSampler2:
    # based directly on the MLP-PLR paper and correspondence with Yury Gorishniy
    def __init__(self, is_classification: bool, train_size: int, num_emb_type: str = 'none'):
        self.is_classification = is_classification
        self.train_size = train_size
        self.num_emb_type = num_emb_type
    ...
    def sample_params(self, seed: int) -> Dict[str, Any]:
        ...
        params = {
            # reduced d_layers
            "module__n_layers": rng.choice(np.arange(1, 17)),
            "module__d_layers": rng.choice(np.arange(1, 1025)),
            # "Note that the size of the first and the last layers are tuned and set separately, while the size for
            # “in-between” layers is the same for all of them." from rtdl paper
            "module__d_first_layer": rng.choice(np.arange(1, 1025)),
            "module__d_last_layer": rng.choice(np.arange(1, 1025)),
            "module__dropout": rng.choice([rng.uniform(0, 0.5)] + [0.]),
            "lr": np.exp(rng.uniform(np.log(5e-5), np.log(5e-3))),
            "optimizer__weight_decay": rng.choice(
                [np.exp(rng.uniform(np.log(1e-6), np.log(1e-3)))] + [0.]
            ),
            # from https://github.com/yandex-research/rtdl-num-embeddings/blob/main/bin/train3.py
            # it seems that MLP-PLR used one-hot encoding  (it's also written in the paper)
            # in
            "module__d_embedding": rng.choice(np.arange(1, 64)),  # we go lower (than 64)
            # because we have smaller datasets with categorical features
            "batch_size": choose_batch_size_rtdl_new(self.train_size),
            "lr_scheduler": False,
            "optimizer": "adamw",
        ...
        if self.num_emb_type != 'none':
            params['module__num_emb_type'] = self.num_emb_type
            params['module__num_emb_dim'] = rng.choice(np.arange(1, 65))  # reduced for RAM reasons
            params['module__num_emb_hidden_dim'] = rng.choice(np.arange(1, 65))  # reduced for RAM reasons
            params['module__num_emb_sigma'] = np.exp(
                rng.uniform(np.log(1e-2), np.log(1e1)))  # reduced based on correspondence with Yury Gorishniy
...
class RTDL_ResNet_ParamSamplerNew:
    ...
    def sample_params(self, seed: int) -> Dict[str, Any]:
        ...
        params = {
            ...
            "module__d_embedding": rng.choice(np.arange(1, 65)),  # use smaller embedding dimensions
            "batch_size": choose_batch_size_rtdl_new(self.train_size),
            "module__activation": "relu",
            "module__normalization": "batchnorm",
            "lr_scheduler": False,
...
class RandomParamsFTTransformerAlgInterface(RandomParamsAlgInterface):
    def _sample_params(self, is_classification: bool, seed: int, n_train: int):
        rng = np.random.default_rng(seed=seed)
        params = {
            "module__n_layers": rng.choice(np.arange(1, 5)),
            "module__d_token": 8 * rng.choice(np.arange(2, 49)),
            ...
            "module__d_ffn_factor": rng.uniform(2 / 3, 8 / 3),
            "module__ffn_dropout": rng.uniform(0.0, 0.5),
            "module__attention_dropout": rng.uniform(0.0, 0.5),
            "module__residual_dropout": rng.choice([rng.uniform(0, 0.2)] + [0.]),
...
class RandomParamsRTDLMLPAlgInterface(SingleSplitAlgInterface):
    ...
    def _create_sub_interface(self, ds: DictDataset, seed: int, n_train: int):
        ...
        if self.fit_params is None:
            hparam_seed = utils.combine_seeds(seed, self.model_idx)
            is_classification = not ds.tensor_infos['y'].is_cont()
            if self.config.get('use_new_rtdl_space', False):
                # use train_size
                train_size = int(0.6 * ds.n_samples)  # the standard benchmark setting
                self.fit_params = [RTDL_MLP_ParamSampler2(is_classification,
                                                          train_size,
                ...
                # old:
                # use n_samples for backwards compatibility (used accidentally)
