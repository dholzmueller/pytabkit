...
# adapted from https://github.com/yandex-research/rtdl-revisiting-models/blob/main/package/README.md
__version__ = "0.0.9"
...
class LinearEmbeddings(nn.Module):
    # this is taken from rtdl_revisiting_models
...
class _NLinear(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # weight.shape: [n_cont, emb_hidden, emb_out]
        # x.shape: [batch_size, n_cont, emb_hidden]
        assert x.ndim == 3
        assert x.shape[-(self.weight.ndim - 1) :] == self.weight.shape[:-1]
        # x = (x[..., None, :] @ self.weight).squeeze(-2)  # old, uses too much RAM
        x = (x.transpose(0, 1) @ self.weight).transpose(0, 1)  # new, uses less RAM
        x = x + self.bias
        return x
