...
def test_numerical_data(n_classes, resnet_or_mlp):
...
@pytest.mark.parametrize("n_classes", [2, 3])
@pytest.mark.parametrize("resnet_or_mlp", ["resnet", "mlp"])
def test_categorical_data(n_classes, resnet_or_mlp):
...
@pytest.mark.parametrize("resnet_or_mlp", ["resnet", "mlp"])
def test_regressor_numerical_categorical(tranformed_target, resnet_or_mlp):
    ...
    assert score > 0.3, f"Regressor R2 score too low with mixed features, got {score}"
...
@pytest.mark.parametrize("resnet_or_mlp", ["resnet", "mlp"])
@pytest.mark.parametrize("n_classes", [2, 3])
def test_all_categorical(regression, resnet_or_mlp, n_classes):
    ...
    from pytabkit.models.sklearn.sklearn_interfaces import Resnet_RTDL_D_Classifier
    model = create_model(regression, resnet_or_mlp, random_state=42)
...
@pytest.mark.parametrize("resnet_or_mlp", ["resnet", "mlp"])
def test_high_cardinality(seed, resnet_or_mlp):
    np.random.seed(seed)
    ...
    reg = create_model(True, resnet_or_mlp, random_state=seed)
